    ...
    do 
    {
    	...
    	switch(CS) 
    	{
			...
			case REAL:
				...
				{
					ungetc(c, fp);
					return  Lex ( LEX_CONSTREAL, 0, stof(buf)); 
				}
				break;
			case NUMB:
				//cout << buf << endl;
				...
				else
				{
					...
					return  Lex ( LEX_CONSTINT, stoi(buf)); 
				}
				break;
			case COM:
				buf.push_back(c);
				...
				else
				{
					...
					return	Lex( LEX_SLASH, (int) LEX_SLASH);
				}
				break;
...
int main() 
{
	...
	try
	{
		while (mannn.get_type() != LEX_EOF)
		{
			mannn = asd.get_lex();
			cout << mannn.get_type() << endl;
			/*
			if (mannn.get_type() >= LEX_FIN)
			{
				cout << mannn.get_value_int() << " " <<  Scanner::TD[mannn.get_value_int() - 24] << endl;
			}
			else
			{
				...
				else if (mannn.get_type() > LEX_FIN)
				{

					cout << mannn.get_value_int() << ' ' << Scanner::TD[mannn.get_value_int()] << endl;
